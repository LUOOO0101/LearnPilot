...
def _run_tool(name: str, args: dict) -> str:
    """根据工具名称和参数执行对应 tool，返回字符串结果。"""
    tools_by_name = {getattr(t, "name", t.__name__): t for t in _TOOLS_LIST}
    ...
    for step in range(max_steps):
        ...
        for tc in response.tool_calls:
            if isinstance(tc, dict):
                ...
                if name is None and "function" in tc:
                    name = tc["function"].get("name") if isinstance(tc["function"], dict) else getattr(tc["function"], "name", None)
                if args is None and isinstance(tc.get("function"), dict) and "arguments" in tc["function"]:
